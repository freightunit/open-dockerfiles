...
if curl -s http://169.254.169.254/latest/meta-data/public-ipv4 > /dev/null ; then
  ...
  echo "Should be run on AWS instance"
  exit 1
fi
...
/usr/local/bin/s3fs $FTP_BUCKET /home/aws/s3bucket -o allow_other -o mp_umask="0022" -o iam_role="$IAM_ROLE" -o curldbg -o endpoint="$AWS_REGION" #-d -d -f -o f2
/usr/local/users.sh
